...
def run_adaptive_implicit_solver(plot_dir, figsize, show,
    ...
    thermo = T9Rho(tp=1e10, rhop=1e6)

    integrator = ThermoNetwork(t9=temperature9, rho=1, tmax=tmax,
    ...
    plt.grid()
    plt.tight_layout()
    filename = f"07.1_t9_{temperature9:.1f}.pdf"
...
def show_mass_fraction_with_thermo_network(plot_dir, figsize, show):
    run_adaptive_implicit_solver(plot_dir=plot_dir, figsize=figsize, show=show,
                                 temperature9=1.1, h=1, tmax=1e25)
