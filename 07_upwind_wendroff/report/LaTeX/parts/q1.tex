...
A useful feature of this Makefile is that is places all the build files (*.o, *.mod and executables) into a separate directory called \code{build}. In addition, we have added ability to create an executable for unit tests by running

\code{\$ make test}

command. This is accomplished by selecting files that end with ``\code{\_test.f90}'' and use them to build the test executable. When the main executable is made, those test source files are excluded.


\subsection{Creating \code{Output} module}

We create an \code{Output} module. It contains \code{write\_output} function that solution array, as well as $x$ and $t$ values to a binary data file. The format of the data file is documented in the README.md. We have chosen a binary file instead of a text file because binary file takes less storage space, and it is faster to read and write.
