...
Finally, on \code{Lines 88-92} we iterate over time indexes (\code{n}) and use the recurrence relation from \autoref{eq_recurrence_relation_heat_eq} to assign the temperature value for the next time index (\code{n + 1}) using the temperatures that were calculated for the previous index of time (\code{n}).
...
Here we want to see how values of $\alpha$ parameter larger than $0.5$ affect the solution. Specifically, we use $\alpha=0.562$, while keeping the same time step $dt=27.4 \ s$ that we used for $nx = 21$. In order to keep $dt$ we increase the number of position values from $nx=21$ to $nx=31$. This increase means that position step sizes decreased from $dx = 0.05 \ m$ to $dx = 0.033 \ m$. The solution is shown on \autoref{fix_solution_nx_20_alpha_0_6_5}.
\begin{figure}[H]
  \centering
